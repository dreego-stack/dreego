...
import (
	...
	client "github.com/dreego-stack/dreego/internal/dreefile/sections/client"
)
...
func GenerateErrorHandler(gen *Generator, file *File, pkgName string, code int, catchPattern string, scopeHash string) (string, string, error) {
	...
	if len(file.Server) > 0 && file.Server[0].Code != "" {
		for line := range strings.SplitSeq(strings.Trim(ir.TranslateMdtohtml(file.Server[0].Code), "\n"), "\n") {
	...
	if file.Body != nil {
		...
		if file.Client != nil {
			clientArtifact, err := client.Client(gen, file)
			if err != nil {
				return "", "", err
			}
