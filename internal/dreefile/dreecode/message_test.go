...
func TestParseMessageExpression(t *testing.T) {
	...
	file, err := parser.NewParser(toks).Parse()
	if err != nil {
...
func TestParseMessageExpressionAllowsGoExpressionsWithSpaces(t *testing.T) {
	key, arguments, err := dreecode.ParseMessageExpression(`home.greeting name=fmt.Sprintf("%s %s", user.First, user.Last) count=len(items)`, 0)
	if err != nil {
		t.Fatal(err)
	}
