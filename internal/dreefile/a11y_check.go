...
func a11yCheck(nodes []ir.TemplateNode) []Diagnostic {
...
func a11yDiagnostics(nodes []ir.TemplateNode) []string {
	ds := a11yCheck(nodes)
...
func collectLabelForIDs(nodes []ir.TemplateNode) map[string]bool {
	ids := map[string]bool{}
	for i := range nodes {
...
func collectLabelForIDsIn(n *ir.TemplateNode, ids map[string]bool) {
...
func scanA11y(nodes []ir.TemplateNode, labelForIDs map[string]bool, diags *[]Diagnostic) {
	for i := range nodes {
		walkA11y(&nodes[i], labelForIDs, diags)
	}
}

func walkA11y(n *ir.TemplateNode, labelForIDs map[string]bool, diags *[]Diagnostic) {
	if n.Type == ir.NodeText {
		checkImgAlt(n, diags)
...
func checkImgAlt(n *ir.TemplateNode, diags *[]Diagnostic) {
	for _, m := range imgRe.FindAllStringSubmatchIndex(n.Content, -1) {
		...
		if !a11yHasAttr(attrs, "alt") {
			pos := n.Pos + m[0]
			line, col := gogen.PosToLineCol(n.SourceText, pos)
...
func checkInputLabel(n *ir.TemplateNode, labelForIDs map[string]bool, diags *[]Diagnostic) {
