...
func validateGeneratedMarkers(disk map[string]string) error {
	paths := slices.Sorted(maps.Keys(disk))
	for _, path := range paths {
		if !isGeneratedFile(disk[path]) {
			return fmt.Errorf("%s was not generated by dreego (missing marker); move it or delete it before running dreego generate", path)
		}
	}
	return nil
}
