...
func TestExtractAttrValuesResolvesExprInQuotedValue(t *testing.T) {
	out := html.ExtractAttrValues(`href="{url}" label="x"`)
	if strings.Contains(out, "{url}") {
		t.Errorf("html.ExtractAttrValues left {url} literal, must resolve to url. got: %s", out)
	}
	if !strings.Contains(out, "url") {
		t.Errorf("html.ExtractAttrValues must contain url expression, got: %s", out)
	}
}
...
func TestAttrValResolvesMultiplePlaceholdersToConcatenation(t *testing.T) {
	out := html.AttrVal(`href="{a}-{b}"`)
	if strings.Contains(out, "a}-{b") {
		t.Errorf("html.AttrVal must split {a}-{b} into separate expressions, got: %s", out)
	}
	if !strings.Contains(out, "a") || !strings.Contains(out, "b") {
		t.Errorf("html.AttrVal must keep both placeholder expressions a and b, got: %s", out)
	}
	if !strings.Contains(out, `"-"`) {
		t.Errorf("html.AttrVal must join placeholders with a literal separator \"-\", got: %s", out)
	}
}
...
func TestGoLiteralPlain(t *testing.T) {
	out := gogen.GoLiteral(`<p>hi</p>`)
	if out != "`<p>hi</p>`" {
		t.Errorf("gogen.GoLiteral plain must wrap in backticks, got: %q", out)
	}
}

func TestGoLiteralContainsBacktick(t *testing.T) {
	out := gogen.GoLiteral("a ` b")
	if strings.Contains(out, "`") && !strings.Contains(out, "\"") {
		t.Errorf("gogen.GoLiteral with backtick must use quotes, got: %q", out)
	}
	if !strings.Contains(out, `"a `) {
		t.Errorf("gogen.GoLiteral with backtick must produce a quoted string, got: %q", out)
	}
}
...
func TestToPascalCaseVariants(t *testing.T) {
	...
	for _, c := range cases {
		if got := gogen.ToPascalCase(c.in); got != c.want {
			t.Errorf("gogen.ToPascalCase(%q) = %q, want %q", c.in, got, c.want)
		}
	}
}
...
func TestAttrValQuotedExpr(t *testing.T) {
	out := html.AttrVal(`prop="{var}"`)
	if out != "var" {
		t.Errorf("html.AttrVal(prop=\"{var}\") = %q, want expression var", out)
	}
}
...
func TestAttrValLiteral(t *testing.T) {
	out := html.AttrVal(`prop="literal"`)
	if out != `"literal"` {
		t.Errorf("html.AttrVal(prop=\"literal\") = %q, want string literal \"literal\"", out)
	}
}
...
func TestAttrValQuotedBoolString(t *testing.T) {
	out := html.AttrVal(`active="false"`)
	if out != `"false"` {
		t.Errorf("html.AttrVal(active=\"false\") = %q, want string literal \"false\"", out)
	}
}
...
func TestConcatPlaceholdersDoesNotEscape(t *testing.T) {
	out := html.ConcatPlaceholders(`href="{a}-{b}"`)
	if strings.Contains(out, "html.EscapeString") {
		t.Errorf("html.ConcatPlaceholders must not escape, escaping is deferred to prop injection. got: %s", out)
	}
	if !strings.Contains(out, "fmt.Sprintf(\"%v\", a)") {
		t.Errorf("html.ConcatPlaceholders must emit fmt.Sprintf for placeholder a, got: %s", out)
	}
	if !strings.Contains(out, "fmt.Sprintf(\"%v\", b)") {
		t.Errorf("html.ConcatPlaceholders must emit fmt.Sprintf for placeholder b, got: %s", out)
	}
	if !strings.Contains(out, `"-"`) {
		t.Errorf("html.ConcatPlaceholders must keep the literal separator \"-\", got: %s", out)
	}
}
