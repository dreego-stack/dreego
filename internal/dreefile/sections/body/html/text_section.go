...
func compTextSection(gen *codegen.State, content string, inSection bool, contextName string) (string, bool, error) {
	...
	for i < len(content) {
		if cur {
			if closeLen := sectionCloseLen(content[i:]); closeLen > 0 {
				...
				parts = append(parts, gogen.GoLiteral(content[start:end]))
				start = end
				i = end
				cur = false
				continue
		...
		if quote != 0 && strings.HasPrefix(content[i:], "{{") {
			closeIdx := dreecode.FindExprEnd(content[i+2:])
			if closeIdx < 0 {
				i++
				continue
			}
		...
		if quote != 0 && strings.HasPrefix(content[i:], "[[") {
			closeIndex := dreecode.FindMessageEnd(content[i+2:])
			if closeIndex < 0 {
				i++
				continue
			}
			...
			key, arguments, err := dreecode.ParseMessageExpression(content[i+2:i+2+closeIndex], i)
			if err != nil && gen != nil {
				return "", cur, err
			}
			if gen == nil {
				parts = append(parts, gogen.GoLiteral(content[i:i+closeIndex+4]))
			} else {
	...
	if start < len(content) {
		parts = append(parts, gogen.GoLiteral(content[start:]))
	}
