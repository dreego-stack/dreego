...
func (g *CompGen) Node(n ir.TemplateNode) (string, error) {
	...
	case ir.NodeVerbatim:
		return fmt.Sprintf("%s.WriteString(%s)", g.Builder, gogen.GoLiteral(n.Content)), nil
	case ir.NodeClientScript:
		clientArtifact, err := clientpkg.Inline(g.Gen, n, g.Component, g.Server)
		if err != nil {
			return "", err
		}
		return strings.TrimSuffix(jsoutput.GenClientTo(clientArtifact, g.Builder, ""), "\n"), nil
	}
...
func (g *CompGen) genComponentCall(n ir.TemplateNode) (string, error) {
	...
	if def == nil {
		return "", fmt.Errorf("%s: unknown component %s", gogen.SourceRef(n.Source, n.Pos), funcName)
	}
