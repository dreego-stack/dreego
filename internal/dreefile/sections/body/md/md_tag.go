...
// ProcessBody applies the Markdown transform to a parsed body section. A body
// whose language is already md converts its whole node stream; every other
// language rewrites inline <md> tag regions into <div> wrappers.
func ProcessBody(nodes []ir.TemplateNode, language string) ([]ir.TemplateNode, error) {
	if language == "md" {
	...
	return TransformInlineMd(nodes)
}
...
func TransformInlineMd(nodes []ir.TemplateNode) ([]ir.TemplateNode, error) {
	...
	for i := 0; i < len(nodes); i++ {
		...
		converted, err := TransformNodes(region)
		if err != nil {
			return nil, err
		}
