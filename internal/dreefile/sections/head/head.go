...
func generate(gen *codegen.State, html, bufName, contextName string) (string, error) {
	...
	for rest != "" {
		...
		if open < 0 {
			out.WriteString(fmt.Sprintf("%s.WriteString(%s)\n", bufName, gogen.GoLiteral(rest)))
			break
		}
		if open > 0 {
			out.WriteString(fmt.Sprintf("%s.WriteString(%s)\n", bufName, gogen.GoLiteral(rest[:open])))
			pos += open
			rest = rest[open:]
		}
		if strings.HasPrefix(rest, "[[") {
			if insideRawElement(html, pos) {
				out.WriteString(fmt.Sprintf("%s.WriteString(%s)\n", bufName, gogen.GoLiteral("[[")))
				pos += 2
				rest = rest[2:]
				continue
			}
			closeIndex := dreecode.FindMessageEnd(rest[2:])
			if closeIndex < 0 {
				return "", fmt.Errorf("unclosed message expression at position %d", pos)
			}
			closeIndex += 2
			key, arguments, err := dreecode.ParseMessageExpression(rest[2:closeIndex], pos)
			if err != nil {
				return "", err
			}
			if gen == nil {
		...
		closeIdx := dreecode.FindExprEnd(rest[2:])
		if closeIdx < 0 {
			out.WriteString(fmt.Sprintf("%s.WriteString(%s)\n", bufName, gogen.GoLiteral(rest)))
			break
		}
		closeIdx += 2
		expr, filters := dreecode.ParseExpression(strings.TrimSpace(rest[2:closeIdx]))
