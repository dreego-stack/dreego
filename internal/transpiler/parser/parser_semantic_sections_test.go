...
func TestParseSemanticSectionLanguages(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		src  string
		want string
		read func(*ir.File) string
	}{
		{name: "server default", src: `<server></server>`, want: "go", read: func(f *ir.File) string { return f.Server[0].Language }},
		...
		{name: "client lua", src: `<client lang="lua"></client>`, want: "lua", read: func(f *ir.File) string { return f.Client.Language }},
	} {
