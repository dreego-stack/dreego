...
func TestValidateUses(t *testing.T) {
	...
	if err := ValidateUses(set, map[string][]string{"hello": {"name"}}); err != nil {
		t.Fatal(err)
	}
	if err := ValidateUses(set, map[string][]string{"missing": {}}); err == nil {
		t.Fatal("expected missing key error")
	}
	if err := ValidateUses(set, map[string][]string{"hello": {}}); err == nil {
...
func TestGoConfig(t *testing.T) {
	...
	got := GoConfig(set, []string{"browser", "default"})
