...
`internal/transpiler/processors` (`md/`, `ts/`, future `lua/`). There is no
external process boundary for first-party languages; third-party processor
support is not planned for v0.3 (removed as speculative API).
...
1. Markdown body processor, which exercises structured body output and protected
   Dreego template placeholders, using stdlib-first parsing.
