...
below are guaranteed to work on a clean machine with Go 1.25 or newer.

## Prerequisites

Dreego requires Go 1.25 or newer. Check your installation:
...
version is older than 1.25, upgrade before continuing — generated code uses
Go 1.25 features and `go mod` directives.
...
| `go: command not found` | Install Go 1.25+ from https://go.dev/doc/install. |
| `go: go.mod requires ... but ...` | Your Go toolchain is older than 1.25. Upgrade. |
