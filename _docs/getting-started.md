...
below are guaranteed to work on a clean machine with Go 1.26 or newer.

## Prerequisites

Dreego requires Go 1.26 or newer. Check your installation:
...
version is older than 1.26, upgrade before continuing — generated code uses
Go 1.26 features and `go mod` directives.
...
| `go: command not found` | Install Go 1.26+ from https://go.dev/doc/install. |
| `go: go.mod requires ... but ...` | Your Go toolchain is older than 1.26. Upgrade. |
