...
The `<server>` block is generated by the transpiler — the target is known at compile time. The developer should have a consistent API, but the targets differ fundamentally.
...
func (c *SSGContext) Metadata() map[string]string { ... } // ONLY SSG, provisional
