...
Every generated `dree.go` starts with a marker line
(`// Code generated by dreego <version> from <source>. DO NOT EDIT. …`).
`generate` only overwrites or deletes a `dree.go` that carries this marker; a
`dree.go` without it is treated as a handwritten file and reported as an error
