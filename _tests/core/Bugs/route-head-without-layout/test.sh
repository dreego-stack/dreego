...
# What: Route with head but no layout file keeps head contents in the rendered fragment
set -e
...
<head><title>No Layout Title</title><script src="route.js"></script></head>
<div><p>hello</p></div>
DREEGO
...
echo "$BODY" | grep -q '<title>No Layout Title</title>' || { echo "FAIL: route head title missing in body: $BODY"; exit 1; }
echo "$BODY" | grep -q 'route.js' || { echo "FAIL: route head script missing in body: $BODY"; exit 1; }
