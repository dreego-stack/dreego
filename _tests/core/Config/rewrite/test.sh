...
# What: Tests that rewrites configured in config.json work at runtime
set -e
...
mkdir -p dreego/routes/new

cat > dreego/routes/new/index.dreego << 'DREEGO'
<div><p>rewritten content</p></div>
DREEGO
...
BODY=$(curl -s http://localhost:$port/old)
echo "$BODY" | grep -q "rewritten content" || { echo "FAIL: expected rewritten content, got $BODY"; exit 1; }
