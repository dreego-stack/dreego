...
func TestDiscoveryIgnoresRoutesOutsideProjectRoot(t *testing.T) {
	...
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/routes/get.dreego":                  `<div><p>real</p></div>`,
	...
	gen, err := os.ReadFile(filepath.Join(dir, "dreego", "gen", "routes.go"))
	if err != nil {
...
func TestDiscoveryIgnoresNestedDreegoProjectRoots(t *testing.T) {
	t.Parallel()
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/routes/get.dreego":        `<div><p>outer</p></div>`,
...
func TestDiscoveryIgnoresComponentsOutsideProjectRoot(t *testing.T) {
	...
	dir := dreegotest.ProjectDir(t, map[string]string{
		...
		"dreego/routes/get.dreego":                  `<div><@Inner/></div>`,
		"vendor/lib/dreego/components/Outer.dreego": "Component Outer ()\n<div><p>outer</p></div>",
	})
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate: %v\n%s", err, out)
	}
	gen, err := os.ReadFile(filepath.Join(dir, "dreego", "gen", "components.go"))
	if err != nil {
...
func TestDiscoveryLayoutLocalCascades(t *testing.T) {
	t.Parallel()
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/layouts/default.dreego":             `<div><html><body><nav>Root</nav>{#slot}</body></html></div>`,
		"dreego/routes/get.dreego":                  `<div><p>home</p></div>`,
		"dreego/routes/blog/get.dreego":             `<div><p>blog</p></div>`,
		"dreego/routes/blog/layouts/default.dreego": `<div><html><body><nav>Blog</nav>{#slot}</body></html></div>`,
	})
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate: %v\n%s", err, out)
	}
...
func TestDiscoveryAmbiguousLayoutsFails(t *testing.T) {
	t.Parallel()
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/layouts/default.dreego": `<div><html><body>{#slot}</body></html></div>`,
		"dreego/layouts/layout.dreego":  `<div><html><body><nav>X</nav>{#slot}</body></html></div>`,
		"dreego/routes/get.dreego":      `<div><p>home</p></div>`,
	})
...
func TestDiscoveryAmbiguousErrorPagesFails(t *testing.T) {
	t.Parallel()
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/routes/404.dreego":         `<div><p>root 404</p></div>`,
		"dreego/routes/(group)/404.dreego": `<div><p>group 404</p></div>`,
		"dreego/routes/get.dreego":         `<div><p>home</p></div>`,
	})
	out, err := dreegotest.RunCLI(t, dir, "generate")
	if err == nil {
...
func TestDiscovery404And500SameDirGenerateAndCheck(t *testing.T) {
	t.Parallel()
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/routes/404.dreego": `<div><p>not found</p></div>`,
		"dreego/routes/500.dreego": `<div><p>server error</p></div>`,
		"dreego/routes/get.dreego": `<div><p>home</p></div>`,
	})
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
...
func TestDiscoveryDeterministicAddMoveDelete(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(dir, "dreego", "routes", "about"), 0755); err != nil {
		t.Fatalf("mkdir about: %v", err)
	}
	if err := os.WriteFile(filepath.Join(dir, "dreego", "routes", "about", "get.dreego"), []byte(`<div><p>about</p></div>`), 0644); err != nil {
		t.Fatalf("write about/get.dreego: %v", err)
	}
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate after add: %v\n%s", err, out)
	}
	gen2, err := os.ReadFile(filepath.Join(dir, "dreego", "gen", "routes.go"))
	if err != nil {
		t.Fatalf("read gen routes.go: %v", err)
	}
	if !strings.Contains(string(gen2), "about") {
		t.Fatalf("added route not discovered: %s", gen2)
	}
	if !strings.Contains(string(gen1["dreego/gen/routes.go"]), "home") {
		t.Fatalf("baseline missing home: %s", gen1["dreego/gen/routes.go"])
	}
	if err := os.Rename(filepath.Join(dir, "dreego", "routes", "about"), filepath.Join(dir, "dreego", "routes", "info")); err != nil {
		t.Fatalf("move about -> info: %v", err)
	}
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate after move: %v\n%s", err, out)
	}
	gen3, err := os.ReadFile(filepath.Join(dir, "dreego", "gen", "routes.go"))
	if err != nil {
		t.Fatalf("read gen routes.go after move: %v", err)
	}
	...
	if err := os.RemoveAll(filepath.Join(dir, "dreego", "routes", "info")); err != nil {
		t.Fatalf("remove info: %v", err)
	}
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate after delete: %v\n%s", err, out)
	}
	gen4, err := os.ReadFile(filepath.Join(dir, "dreego", "gen", "routes.go"))
	if err != nil {
		t.Fatalf("read gen routes.go after delete: %v", err)
	}
...
func TestDiscoveryGeneratesRoutesNamedStaticAndGen(t *testing.T) {
	...
	dir := dreegotest.ProjectDir(t, map[string]string{
		"dreego/routes/get.dreego":             `<div><p>home</p></div>`,
		"dreego/routes/static/get.dreego":      `<div><p>static route</p></div>`,
		"dreego/routes/gen/get.dreego":         `<div><p>gen route</p></div>`,
		"dreego/routes/blog/static/get.dreego": `<div><p>blog static</p></div>`,
	})
	if out, err := dreegotest.RunCLI(t, dir, "generate"); err != nil {
		t.Fatalf("generate: %v\n%s", err, out)
	}
	...
	for _, want := range []string{"HandleStatic", "HandleGen", "HandleBlogStatic"} {
		if !strings.Contains(string(gen), want) {
			t.Fatalf("expected %s handler in generated routes.go: %s", want, gen)
		}
	}
}
