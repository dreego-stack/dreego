...
func TestBugHeadDedupeCaseInsensitiveTitle(t *testing.T) {
	t.Parallel()
...
<body><h1>Page</h1></body>`,
	...
	_, body := c.Get(t, "/")
	if n := strings.Count(strings.ToLower(body), "<title>"); n != 1 {
...
func TestBugHeadDedupeCaseInsensitiveMetaDescription(t *testing.T) {
	t.Parallel()
	c := dreegotest.Serve(t, map[string]string{
		"www/layouts/default.dreego": `<head>
...
<body><h1>Page</h1></body>`,
	...
	_, body := c.Get(t, "/")
	if n := strings.Count(strings.ToLower(body), `name="description"`); n != 1 {
...
func TestBugBodyLevelHeadDedupeCaseInsensitive(t *testing.T) {
	t.Parallel()
	c := dreegotest.Serve(t, map[string]string{
