...
## Current Phase: DreeJS foundation
...
Lua, the v0.8 coordinated module boundaries, and the experimental Wails v3
Phase 1 adapter with its coordinated v0.9 module boundary release are released.
The current phase is the DreeJS foundation. Wails v3 Phase 2 waits for an
upstream stable Wails v3 release and evidence from the Phase 1 reference
application. Every change lands through a pull request with one unique
`.changes/*.md` file. `version: none` files remain pending until a later
`version: patch` change triggers a release.
...
- SSR is the production web host. The render foundation is target-neutral for
  tests and the Wails adapter. Static site generation is not planned.
...
- Go 1.27+, prefer the standard library
...
support the first-party Wails adapter. DreeJS is the optional browser and
